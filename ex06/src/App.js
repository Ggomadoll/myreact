...
//Provider : state 값을 어떤 컴포넌트와 공유, 제공 할건지를 정의!

// reducer : store에 접근해서 state에 관련된 작업을 처리할 기능
// --> 현재의 상태값, 작업(action)
function reducer (currentState, action) {
  if(currentState===undefined) { // reducer가 처음 store에 적용 될때
    return {
  ...
  const [number, setNumber] = useState(3);
    ...
    <div id='grid'>
      <Provider store={store}>
        {/* 위에서 만들어 놓은 store와 연결시킨뒤 해당 내용을 하위 컴포넌트에 제공 하겠습니다! 
        하위 컴포넌트가 사용하고 있는 다른 컴포넌트에도 store*/}
